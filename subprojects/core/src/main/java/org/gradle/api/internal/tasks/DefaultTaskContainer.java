...
public class DefaultTaskContainer extends DefaultTaskCollection<Task> implements TaskContainerInternal {
    ...
    private final Map<String, Provider<? extends Task>> placeholders = Maps.newLinkedHashMap();
    ...
    private <T extends Task> T addTask(T task, boolean replaceExisting) {
        ...
        if (placeholders.remove(name) != null) {
            if (modelNode != null) {
    ...
    private void flushPlaceholders() {
        for (Iterator<Provider<? extends Task>> iterator = placeholders.values().iterator(); iterator.hasNext();) {
            iterator.next().get();
            iterator.remove();
        }
    }
    ...
    private boolean maybeCreateTasks(String name) {
        Provider<?> placeholder = placeholders.remove(name);
        if (placeholder != null) {
            placeholder.get();
            return true;
        }
    ...
    public void whenElementKnown(Action<? super ElementInfo<Task>> action) {
        ...
        for (Map.Entry<String, Provider<? extends Task>> entry : placeholders.entrySet()) {
